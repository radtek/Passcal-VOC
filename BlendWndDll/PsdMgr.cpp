...
void CPsdMgr::OnOK()
{
	...
	if (m_nLevel < 0)
	{
		ShowMsgBox(_T("未选择任何权限。"), m_strTitle, MB_OK | MB_ICONERROR);
	...
	if (m_bChangePsd)
	{
		...
		if (strNew.IsEmpty())
		{
			ShowMsgBox(_T("新密码不能为空！请重新输入。"), _T("修改密码"), MB_OK | MB_ICONERROR);
		...
		{
			ShowMsgBox(_T("确认密码和新密码不同！请重新输入。"), _T("修改密码"), MB_OK | MB_ICONERROR);
		...
		if (ERROR_SUCCESS == regKey.Open(HKEY_LOCAL_MACHINE, strKeyName))
		{
			...
			{
				ShowMsgBox(_T("修改密码成功！"), _T("修改密码"), MB_OK | MB_ICONINFORMATION);

				regKey.Close();
			}
			else
			{
				ShowMsgBox(_T("新密码写入失败！"), _T("修改密码"), MB_OK | MB_ICONERROR);
		...
		{
			ShowMsgBox(_T("访问密码文件失败！"), _T("修改密码"), MB_OK | MB_ICONERROR);
...
BOOL CPsdMgr::_ReadCode(void)
{
	...
	if (FALSE == bOpenSuccess)
	{
		...
		{
			ShowMsgBox(_T("访问密码文件失败！"), _T("密码"), MB_OK | MB_ICONERROR);
